...
class BookQualityAuditor:
    ...
    def check_generic_titles(self, chapters):
        ...
        for chapter in chapters:
            title_zh = chapter.get("title_zh", "").lower()
            title_en = chapter.get("title_en", "").lower()
            
            for pattern in self.generic_patterns:
                if pattern.lower() in title_zh or pattern.lower() in title_en:
                    generic_count += 1
                    break
...
## 统计总结
- ✅ 通过 (PASS): """ + str(len(self.results['PASS'])) + """ 本
- ⚠️ 部分通过 (PARTIAL): """ + str(len(self.results['PARTIAL'])) + """ 本  
- ❌ 不通过 (FAIL): """ + str(len(self.results['FAIL'])) + """ 本
- **总计**: """ + str(len(self.results['PASS']) + len(self.results['PARTIAL']) + len(self.results['FAIL'])) + """ 本
...
"""
        ...
        for book in self.results["PASS"]:
            report += f"| {book['id']} | {book['title_zh']} ({book['title_en']}) | {book['current_chapters']} | {book['expected_chapters']} | {book['problems']} |\n"
...
def main():
    ...
    print(f"审查完成！")
    print(f"✅ 通过: {len(auditor.results['PASS'])} 本")
    print(f"⚠️ 部分通过: {len(auditor.results['PARTIAL'])} 本")
    print(f"❌ 不通过: {len(auditor.results['FAIL'])} 本")
