...
export default function AuthorBooksClient({ authorName }: { authorName: string }) {
  const authorBooks = books.filter((b) => b.author_zh === authorName);
  const authorEn = authorBooks[0]?.author_en || '';

  if (authorBooks.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-8 text-center">
        <p className="text-[var(--color-text-secondary)]">未找到该作者的书籍</p>
        <Link href="/library/author" className="text-[var(--color-accent)] hover:underline text-sm mt-4 inline-block">
          ← 返回作者列表
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="text-center mb-8">
        ...
        {authorEn && (
          <p className="text-sm text-[var(--color-text-secondary)] italic mb-2">{authorEn}</p>
        )}
        <p className="text-sm text-[var(--color-text-secondary)]">
          共 {authorBooks.length} 本书
        </p>
      </div>
        ...
        {authorBooks.map((book, i) => (
          <Link key={book.id} href={`/library/${book.id}`} className="block group">
            <div className="rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all hover:-translate-y-0.5">
              <div
                className={`bg-gradient-to-br ${gradients[i % gradients.length]} p-5 aspect-[3/4] flex flex-col justify-between text-white relative`}
              >
                ...
                <span className="text-xs opacity-60 uppercase tracking-wider line-clamp-1">
                  {book.author_en}
                </span>
                <div>
