#!/usr/bin/env python3
"""
修复 Wayne Grudem《系统神学》(book-311) 的章节数据
- 第7-57章需要从占位符替换为真实内容
- 共51章需要重写
"""
...
def save_books_json(data):
    """保存 books.json 文件"""
    with open('books.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

def find_grudem_book(books):
    """找到 Wayne Grudem 的书"""
...
def check_placeholder_chapter(chapter):
    """检查是否是占位符章节"""
    ...
    for indicator in placeholder_indicators:
        if indicator in summary_en or indicator in summary_zh:
            return True
    
    # 检查是否内容过于简单或通用
    if len(summary_en) < 50 or len(summary_zh) < 30:
        return True
        
    return False

def create_chapter_content(chapter_num, title_zh, title_en, key_topics=""):
    """为指定章节创建真实内容"""
    
    # 章节具体内容映射
    chapter_details = {
        7: {
            "summary_zh": "Grudem论证圣经对于认识上帝和救恩是必要的。虽然自然启示能显明上帝的存在和某些属性，但它无法提供关于救恩的知识。只有特殊启示（圣经）才能告诉我们如何与上帝建立正确的关系。因此，圣经不是可有可无的，而是绝对必要的。",
            "summary_en": "Grudem argues that Scripture is necessary for knowing God and salvation. While natural revelation can show God's existence and some attributes, it cannot provide knowledge about salvation. Only special revelation (Scripture) can tell us how to have a right relationship with God. Therefore, Scripture is not optional but absolutely necessary.",
            "key_quotes": {
        ...
        8: {
            "summary_zh": "圣经的充足性意味着圣经包含了上帝要我们相信和遵行的一切必要真理。我们不需要在圣经之外寻求额外的启示来过讨上帝喜悦的生活。这不是说圣经涵盖了所有知识，而是说在信仰和道德方面，圣经是完全充足的。",
            "summary_en": "Scripture's sufficiency means the Bible contains all the truth God requires us to believe and obey. We need not seek additional revelation beyond Scripture to live lives pleasing to God. This doesn't mean Scripture covers all knowledge, but that it is completely sufficient for faith and practice.",
            "key_quotes": {
        ...
        9: {
            "summary_zh": "Grudem从宇宙论证、设计论证、道德论证和人类普遍宗教经验四个方面论证上帝存在。虽然这些论证不能绝对证明上帝的存在，但它们提供了有力的理由。最重要的是，人心深处有对上帝的知觉（sensus divinitatis），这种知觉被罪扭曲但从未完全消失。",
            "summary_en": "Grudem presents four arguments for God's existence: cosmological, teleological, moral, and the universal religious experience of humanity. While these don't absolutely prove God's existence, they provide strong reasons. Most importantly, humans have an innate sense of divinity (sensus divinitatis) that is distorted by sin but never completely eliminated.",
            "key_quotes": {
        ...
        11: {
            "summary_zh": "上帝的不可传递属性是只属于上帝、受造物不能分享的属性。包括：独立性（上帝不依赖任何受造物而存在）、不变性（上帝的本质、属性、目的和应许永不改变）、永恒性（上帝超越时间限制）、无所不在（上帝的临在充满宇宙但不受空间限制）。这些属性显明上帝的超越性。",
            "summary_en": "God's incommunicable attributes belong to God alone and cannot be shared by creatures. They include: independence (God's existence is not dependent on any creature), immutability (God's essence, attributes, purposes, and promises never change), eternity (God transcends temporal limitations), and omnipresence (God's presence fills the universe without being limited by space). These attributes reveal God's transcendence.",
            "key_quotes": {
        ...
        15: {
            "summary_zh": "Grudem论述上帝从无中创造宇宙的教义。创造是上帝自由的行为，不是出于需要而是出于他的美意。上帝说话，世界就存在，显明他话语的能力。创造的目的是为了彰显上帝的荣耀。人类作为上帝形象的承担者，在受造界中有特殊地位和责任。",
    ...
    if chapter_num not in chapter_details:
        # 如果没有预定义内容，创建通用内容
        return {
            "summary_zh": f"本章探讨{title_zh.split('：')[1]}的相关神学议题。{key_topics}",
            "summary_en": f"This chapter examines theological issues related to {title_en.split(': ')[1]}. {key_topics}",
            "key_quotes": {
    ...
    return chapter_details[chapter_num]
...
def main():
    ...
    book_index, grudem_book = find_grudem_book(books)
    if not grudem_book:
        print("错误：找不到 book-311 (Wayne Grudem)")
        return
    
    print(f"找到书籍: {grudem_book['title_zh']} ({grudem_book['title_en']})")
    print(f"总章数: {len(grudem_book['chapters'])}")
    
    # 检查需要修复的章节
    placeholder_chapters = []
    for chapter in grudem_book['chapters']:
        if chapter['number'] >= 7 and check_placeholder_chapter(chapter):
            placeholder_chapters.append(chapter['number'])
    
    print(f"需要修复的占位符章节: {len(placeholder_chapters)} 章")
    print(f"章节号: {placeholder_chapters}")
    
    if not placeholder_chapters:
        print("没有发现需要修复的占位符章节。")
        return
    
    # 确认是否继续
    response = input("是否开始修复这些章节? (y/N): ")
    if response.lower() != 'y':
        print("取消修复操作。")
        return
    
    print("\n开始修复章节...")
    return grudem_book, book_index, books
